  ...
  return (
    <div className={styles.MemeForm} data-testid="MemeForm">
      ...
      >
        ...
        <br />
        <select name="image" id="image" value={meme.imageId} onChange={(evt) => onMemeChange({ ...meme, imageId: Number(evt.target.value)})}>
          {images.map((element, position) => (
            <option key={"si" + position} value={element.id}>{element.name}</option>
          ))}
        </select>
        <hr />
        <label htmlFor="text">
